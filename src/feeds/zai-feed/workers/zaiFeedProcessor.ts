...
import { MySQL, Paging } from 'src/utils/mysql';
import mysql from 'mysql2/promise';
import { format } from 'mysql2'
import TSVData from 'src/feeds/naver-feed/tsv-data';
import TSVFormat from '../../naver-feed/tsv-format'
import Constants from '../../naver-feed/constants'

const constants = new Constants();

export function selectSellableItemsPagedQuery(paging: Paging): string {
    return `
        SELECT
            idx
        FROM
...
export function selectZaiFeedQuery(itemIds): string {
    return format(`
        SELECT STRAIGHT_JOIN ii.idx AS 'id',
            ...
            (
                SELECT 
                    SUBSTRING_INDEX(GROUP_CONCAT(REPLACE(ig.item_image_url, ',', '%2C') SEPARATOR '|'), '|', 10)
                FROM
                    item_image ig
                WHERE
                    ig.item_id = ii.idx
                    ...
                    SUBSTRING_INDEX(
                        GROUP_CONCAT(
                            CONCAT(
                                ...
                                '^',
                                CEIL(
                                    (ip.final_price + IFNULL(i.optional_price, 0)) * 0.97 / 100
                                ) * 100
                            ) SEPARATOR '|'
                        ),
                        ',',
...
export async function makeRow(row: mysql.RowDataPacket): Promise<TSVData> {
...
export async function selectSellableItems(offset: number, chunkSize: number): Promise<number[]> {
    const result: number[] = (
        await MySQL.execute(selectSellableItemsPagedQuery({offset: offset, limit: chunkSize}))
    )[0]
        .map((row: mysql.RowDataPacket) => row['idx']);
    return result;
}

export async function selectZaiFeed(itemIds: number[]): Promise<TSVData[]> {
    const queryResult: mysql.RowDataPacket[] = (
        await MySQL.execute(selectZaiFeedQuery(itemIds))
    )[0];
    const result = (
        await Promise.all(
            queryResult
                .map((row: mysql.RowDataPacket) => row.option_detail && row.category_name1 && row.category_name2 && row.category_name3)
                .map(makeRow))).filter(row => row)
    // const result: TSVData[]  = queryResult.map((row: mysql.RowDataPacket) => {
    //     return row.option_detail && row.category_name1 && row.category_name2 && row.category_name3;
    // }).map(async (row: mysql.RowDataPacket) => {
    //     return (await makeRow(row));
    // })
    return result;
}

async function main() {
    const offset: number = workerData.offset;
    const chunkSize: number = workerData.chunkSize;
    console.log(`worker thread ${Math.floor(offset/chunkSize)} running`);
    const sellableItemIds: number[] = await selectSellableItems(offset, chunkSize);
    const zaiFeedRows: TSVData[] = await selectZaiFeed(sellableItemIds);
    parentPort.postMessage(zaiFeedRows);
}
