...
export class ZaiFeed implements iFeed {
	...
	async getTsv(delimiter = '\t'): Promise<fs.ReadStream> {
		...
		const listRaw = await MySQL.execute('SELECT idx FROM item_info WHERE is_sellable')
		const list = listRaw.map(row => row.idx)
		const chunkedList = chunk(list, 100000)

		const tsvDataList = await Promise.all(chunkedList.map(async list => {
			const data = await MySQL.execute(ZaiFeed.query(list))
